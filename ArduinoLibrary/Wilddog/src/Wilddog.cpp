...
{ 
    return _wd_connect.platform_Busying();
}
...
int Wilddog::connect_init(const char *p_url)
{
  ...
  return _wd_connect.cmd_send(_CMD_INIT_WILDDOG,p_url); 
}
**/
/*
* 
...
{
    if(wilddog_index)
        return _wd_connect.connect_send(_CMD_AUTH,p_token,p_host,f_callback,arg);  
    else
        return -1;
}
