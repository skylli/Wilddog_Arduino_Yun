...
static const char *parse_value(Wilddog_Node_T *item,const char *value);
...
static const char *parse_string(Wilddog_Node_T *item,const char *str)
{
	...
	out=(char*)wmalloc(len+1);	/* This is how long we need for the string, roughly. */
	if (!out) return 0;
	...
	*ptr2=0;
	if (*ptr=='\"') ptr++;
	//memcpy(item->p_wn_value, out, strlen(out));
	item->p_wn_value=out;
...
static const char *parse_number(Wilddog_Node_T *item,const char *num)
{
...
static const char *parse_array(Wilddog_Node_T *item,const char *value)
{
	...
	if (!item->p_wn_child) return 0;		 /* memory fail */
...
static const char *parse_object(Wilddog_Node_T *item,const char *value)
{
	Wilddog_Node_T *child;
	...
	if (!value) return 0;
	child->p_wn_key=child->p_wn_value;child->p_wn_value=0;
...
{
	const char *end=0;
	Wilddog_Node_T *c=wilddog_node_createObject("/");
	if (!c) return 0;       /* memory fail */

	end=parse_value(c,skip(value));
	if (!end)	{wilddog_node_delete(c);return 0;}	/* parse failure. ep is set. */
